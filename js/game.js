...
const gameState = new Proxy({
        ...
        currentScreen: null,
        // // 探索内サブ状態
        // explorePhase: "is_idle" | "is_rolling" | "is_event" | "is_event_choice",
        // // 戦闘内サブ状態
        // combatPhase: "start" | "command_waiting" | "enemy_act" | "result",
...
async function acquireItem(item) {
    if (player.item_slot.length < 20) {
        ...
        updateUI();
        // saveGame(player);// リロードとかの処理がちょっと怪しいので無効化
        return true;
    } else {
...
async function advance() {
    ...
    advanceButton.disabled = true; // ボタンを一時的に無効化
    advanceButton.classList.add("hidden");
    ...
    player.currentEventCompleted = false; // 新しいマスに移動したのでイベントは未完了
    updatePlayerStatus();
...
async function attack() {
    ...
    attackButton.disabled = false; // ターン終了後、攻撃ボタンを有効化
    attackButton.classList.remove("hidden"); // hidden プロパティではなくクラスを使用
}
